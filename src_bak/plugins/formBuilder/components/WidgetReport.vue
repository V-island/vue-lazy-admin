  ...
  >
                ...
                <el-dropdown size="default" trigger="click">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item
                        command="insertleft"
                        @click.stop="handleInsertLeft"
                        >{{
                          $t('fm.tooltip.insertcolumnleft')
                        }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="insertright"
                        @click.stop="handleInsertRight"
                        >{{
                          $t('fm.tooltip.insertcolumnright')
                        }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="insertbefore"
                        @click.stop="handleInserBefore"
                        >{{
                          $t('fm.tooltip.insertrowbefore')
                        }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="insertafter"
                        @click.stop="handleInserAfter"
                        >{{ $t('fm.tooltip.insertrowafter') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="right"
                        @click.stop="handleRight(column)"
                        :disabled="!showRight"
                        divided
                        >{{ $t('fm.tooltip.mergeright') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="bottom"
                        @click.stop="handleBottom(column)"
                        :disabled="!showBottom"
                        >{{ $t('fm.tooltip.mergedown') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="splitcolumns"
                        @click.stop="handleSplitColumn"
                        divided
                        :disabled="!showSplitColumn"
                        >{{ $t('fm.tooltip.splitcolumns') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="splitrows"
                        @click.stop="handleSplitRow"
                        :disabled="!showSplitRow"
                        >{{ $t('fm.tooltip.splitrows') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="removecolumn"
                        @click.stop="handleRemoveColumn"
                        :disabled="!showRemoveColumn"
                        divided
                        >{{ $t('fm.tooltip.deletecolumn') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        command="removerow"
                        @click.stop="handleRemoveRow"
                        :disabled="!showRemoveRow"
                        >{{ $t('fm.tooltip.deleterow') }}</el-dropdown-item
                      >
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
    ...
    >
      <i
        ...
        :title="$t('fm.tooltip.addrow')"
      ></i>
      <i
        class="iconfont icon--charulie"
        @click.stop="handleAddColumns(index)"
        :title="$t('fm.tooltip.addcolumn')"
      ></i>
      <i
        class="iconfont icon-icon_clone"
        @click.stop="handleReportClone(index)"
...
export default {
  ...
  computed: {
    showRight() {
      if (this.selectR >= 0 && this.selectC >= 0) {
        const currentRowspan =
          this.element.rows[this.selectR].columns[this.selectC].options.rowspan
        const currentColspan =
          this.element.rows[this.selectR].columns[this.selectC].options.colspan

        if (
          this.selectC + currentColspan <
          this.element.rows[this.selectR].columns.length
        ) {
          const nextColumn =
            this.element.rows[this.selectR].columns[
              currentColspan + this.selectC
            ]

          if (
            !nextColumn.options.invisible &&
            currentRowspan == nextColumn.options.rowspan
          ) {
            return true
          }
        }
    ...
    showBottom() {
      if (this.selectR >= 0 && this.selectC >= 0) {
        ...
        if (currentRowspan + this.selectR < this.element.rows.length) {
          const nextColumn =
            this.element.rows[currentRowspan + this.selectR].columns[
              this.selectC
            ]

          if (
            !nextColumn.options.invisible &&
            currentColspan == nextColumn.options.colspan
          ) {
            return true
          }
        }
      ...
      if (
        ...
        if (currentRowspan == 1) {
          ...
          for (; i < this.element.rows[this.selectR].columns.length; i++) {
            if (
              currentRowspan !=
                this.element.rows[this.selectR].columns[i].options.rowspan ||
              this.element.rows[this.selectR].columns[i].options.invisible
            ) {
              return false
            }
          }
    ...
    showRemoveColumn() {
      if (
        this.selectR >= 0 &&
        this.selectC >= 0 &&
        this.element.rows[this.selectR].columns.length > 1
      ) {
        const currentColspan =
          this.element.rows[this.selectR].columns[this.selectC].options.colspan
        if (currentColspan == 1) {
          ...
          for (; i < this.element.rows.length; i++) {
            if (
              currentColspan !=
                this.element.rows[i].columns[this.selectC].options.colspan ||
              this.element.rows[i].columns[this.selectC].options.invisible
            ) {
              return false
            }
          }
    ...
    showSplitColumn() {
      if (this.selectR >= 0 && this.selectC >= 0) {
        return (
          this.element.rows[this.selectR].columns[this.selectC].options
            .colspan > 1
        )
      }
      return false
  ...
  methods: {
    ...
    handleSelectItemWidget(rowIndex, colIndex) {
      this.$emit(
        'update:select',
        this.data.list[this.index].rows[rowIndex].columns[colIndex]
      )
    ...
    handleWidgetItemAdd($event, element, rowIndex, colIndex) {
      ...
      element.rows[rowIndex].columns[colIndex].list[newIndex] = _.cloneDeep(
      ...
      element.rows[rowIndex].columns[colIndex].list[newIndex] = {
        ...
        key: element.rows[rowIndex].columns[colIndex].list[newIndex].key
          ? element.rows[rowIndex].columns[colIndex].list[newIndex].key
          : key,
        model: element.rows[rowIndex].columns[colIndex].list[newIndex].model
          ? element.rows[rowIndex].columns[colIndex].list[newIndex].model
          : element.rows[rowIndex].columns[colIndex].list[newIndex].type +
            '_' +
            key,
        rules: element.rows[rowIndex].columns[colIndex].list[newIndex].rules
          ? [...element.rows[rowIndex].columns[colIndex].list[newIndex].rules]
          : [],
      }

      if (
        element.rows[rowIndex].columns[colIndex].list[newIndex].type == 'report'
      ) {
        element.rows[rowIndex].columns[colIndex].list[newIndex].rows =
          generateKeyToTD(
            element.rows[rowIndex].columns[colIndex].list[newIndex].rows
          )
      }
      ...
      if (
        element.rows[rowIndex].columns[colIndex].list[newIndex].type == 'grid'
      ) {
        element.rows[rowIndex].columns[colIndex].list[newIndex].columns =
          generateKeyToCol(
            element.rows[rowIndex].columns[colIndex].list[newIndex].columns
          )
      }
    ...
    handleAddRow(index) {
      const columns =
        this.data.list[index].rows[this.data.list[index].rows.length - 1]
          .columns.length
    ...
    insertColumns(insertIndex) {
      for (let i = 0; i < this.element.rows.length; i++) {
        ...
        if (
          ...
          this.element.rows[i].columns[insertIndex].options.invisible
        ) {
          ...
          // 插入隐藏列后，进行标记计算
          const beforeColspan =
            this.element.rows[i].columns[prevIndex].options.colspan
    ...
    insertRow(insertIndex) {
      ...
      for (let i = 0; i < this.element.rows[0].columns.length; i++) {
        ...
        if (
          prevIndex != insertIndex &&
          this.element.rows[insertIndex].columns[i].options.invisible
        ) {
          rowArray.push({
            type: 'td',
            list: [],
          ...
          this.element.rows[prevIndex].columns[i].options.rowspan =
            this.element.rows[prevIndex].columns[i].options.rowspan + 1
        } else {
          rowArray.push({
            type: 'td',
            list: [],
      ...
      this.$nextTick(() => {
        for (
          let i = 0;
          i < this.element.rows[insertIndex].columns.length;
          i++
        ) {
    ...
    handleInsertRight() {
      const insertColIndex =
        this.element.rows[this.selectR].columns[this.selectC].options.colspan >
        1
          ? this.findNextColIndex(this.selectR, this.selectC + 1)
          : this.selectC + 1
    ...
    handleInserAfter() {
      const insertRowIndex =
        this.element.rows[this.selectR].columns[this.selectC].options.rowspan >
        1
          ? this.findNextRowIndex(this.selectR + 1, this.selectC)
          : this.selectR + 1
    ...
    handleRight(column) {
      ...
      const nextColspan =
        this.element.rows[this.selectR].columns[nextC].options.colspan
    ...
    handleBottom(column) {
      ...
      const nextRowspan =
        this.element.rows[nextR].columns[this.selectC].options.rowspan
    ...
    handleSplitColumn() {
      ...
      for (let i = 0; i < currentColspan; i++) {
        ...
        this.element.rows[this.selectR].columns[
          this.selectC + i
        ].options.rowspan = currentRowspan
    ...
    handleSplitRow() {
      ...
      for (let i = 0; i < currentRowspan; i++) {
        this.element.rows[this.selectR + i].columns[
          this.selectC
        ].options.invisible = false
        this.element.rows[this.selectR + i].columns[
          this.selectC
        ...
        this.element.rows[this.selectR + i].columns[
          this.selectC
        ].options.colspan = currentColspan
    ...
    findPrevRowIndex(rowIndex, colIndex) {
      if (
        rowIndex >= this.element.rows.length ||
        !this.element.rows[rowIndex].columns[colIndex].options.markRow
      ) {
        return rowIndex
      } else {
    ...
    findPrevColIndex(rowIndex, colIndex) {
      if (
        colIndex >= this.element.rows[rowIndex].columns.length ||
        !this.element.rows[rowIndex].columns[colIndex].options.markCol
      ) {
        return colIndex
      } else {
    ...
    handleRemoveRow() {
      ...
      for (let j = 0; j < this.element.rows[this.selectR].columns.length; j++) {
        const currentRowspan =
          this.element.rows[this.selectR].columns[j].options.rowspan
        const currentcolspan =
          this.element.rows[this.selectR].columns[j].options.colspan
        const invisible =
          this.element.rows[this.selectR].columns[j].options.invisible

        if (invisible) {
          const prevIndex = this.findPrevRowIndex(this.selectR, j)

          if (
            prevIndex != this.selectR &&
            this.element.rows[prevIndex].columns[j].options.rowspan > 1
          ) {
            this.element.rows[prevIndex].columns[j].options.rowspan =
              this.element.rows[prevIndex].columns[j].options.rowspan - 1
          }
        } else if (currentRowspan > 1) {
          this.element.rows[this.selectR + 1].columns[j].list = [
            ...this.element.rows[this.selectR].columns[j].list,
          ]
          this.element.rows[this.selectR + 1].columns[
            j
          ].options.invisible = false
          this.element.rows[this.selectR + 1].columns[j].options.rowspan =
            currentRowspan - 1
          this.element.rows[this.selectR + 1].columns[j].options.colspan =
            currentcolspan
    ...
    handleRemoveColumn() {
      ...
      for (let i = 0; i < this.element.rows.length; i++) {
        const currentColspan =
          this.element.rows[i].columns[this.selectC].options.colspan
        const currentRowspan =
          this.element.rows[i].columns[this.selectC].options.rowspan
        const invisible =
          this.element.rows[i].columns[this.selectC].options.invisible
        ...
        if (invisible) {
          ...
          if (
            prevIndex != this.selectC &&
            this.element.rows[i].columns[prevIndex].options.colspan > 1
          ) {
            this.element.rows[i].columns[prevIndex].options.colspan =
              this.element.rows[i].columns[prevIndex].options.colspan - 1
          }
        } else if (currentColspan > 1) {
          this.element.rows[i].columns[this.selectC + 1].list = [
            ...this.element.rows[i].columns[this.selectC].list,
          ]
          this.element.rows[i].columns[
            this.selectC + 1
          ].options.invisible = false
          this.element.rows[i].columns[this.selectC + 1].options.colspan =
            currentColspan - 1
          this.element.rows[i].columns[this.selectC + 1].options.rowspan =
            currentRowspan
