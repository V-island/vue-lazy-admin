<template>
    <div class="page">
      <el-form  :inline="true" class="query-form m-b-10" ref="searchForm" :model="searchForm" @keyup.enter="refreshList()" @submit.prevent>
            <!-- 搜索框-->
		     <el-form-item prop="procInsName">
                <el-input  v-model="searchForm.procInsName" placeholder="标题" clearable></el-input>
		     </el-form-item>
          <el-form-item>
      ...
      <div class="jp-table">
        <vxe-toolbar ref="flowCopyToolbar" :refresh="{query: refreshList}" export print custom>
          <template #buttons>
              <el-button  type="danger"  v-if="_hasPermission('extension:flowCopy:del')"  icon="delete" @click="del()"
       ...
       <div class="jp-table-body">
            <vxe-table
                ...
                <vxe-column  title="抄送日期" field="createTime" sortable> </vxe-column>
                ...
                <vxe-column title="操作" width="150px" type="html" fixed="right" align="center">
                  <template #default="scope">
                      <el-button v-if="_hasPermission('extension:flowCopy:view')"  type="primary" text icon="view" size="small" @click="detail(scope.row)">查阅审批</el-button>
                      <el-button v-if="_hasPermission('extension:flowCopy:del')"  type="danger" text size="small" icon="delete" @click="del(scope.row.id)">删除</el-button>
                  </template>
                </vxe-column>
            </vxe-table>
            <vxe-pager
