    ...
    <el-aside width="300px" class="data-source-aside">
      <el-container>
        <el-header height="42px">
          <el-button type="text" size="default" @click="handleAdd">
            <i
        ...
        <el-main>
          ...
          >
            ...
            >
              <template #title>
                <div>
                  <span class="data-source-menu-i" :class="item.method">{{
                    item.method
                  }}</span>
        ...
        <el-header height="42px">
          <div class="data-source-action">
            <el-button type="primary" size="default" @click="handleSave">{{
              $t('fm.datasource.config.save')
            }}</el-button>
            <el-button size="default" @click="handleTest">{{
              $t('fm.datasource.config.test')
            }}</el-button>
            <el-button size="default" @click="handleCancal">{{
              $t('fm.datasource.config.cancel')
            }}</el-button>
          </div>
        </el-header>
        <el-main>
          <el-form
            ...
            :model="formData"
            :rules="formRules"
            :label-width="$i18n.locale == 'zh-cn' ? '79px' : '119px'"
            size="default"
          ...
          >
            ...
            <el-form-item
              :label="$t('fm.datasource.edit.method')"
              prop="method"
            >
            ...
            <el-form-item
              :label="$t('fm.datasource.edit.auto')"
              :label-width="$i18n.locale == 'zh-cn' ? '175px' : '340px'"
            >
            ...
            <el-form-item :label="$t('fm.datasource.edit.response')">
              <el-collapse :modelValue="['1', '2', '3']">
                <el-collapse-item
                  name="1"
                  :title="$t('fm.datasource.edit.willRequest')"
                >
                  <div class="code-desc" v-if="$i18n.locale == 'zh-cn'">
                    // config: 发出请求的可用配置选项;
                  </div>
                  <div class="code-desc" v-if="$i18n.locale == 'zh-cn'">
                    // 通过 config.url 可以更改请求地址，通过 config.headers
                    可以更改请求头部
                  </div>
                  <div class="code-desc" v-if="$i18n.locale == 'zh-cn'">
                    // 通过 config.data 可以更改发送的数据，（GET
                    请求不适用，需要更改 config.params）
                  </div>
                  <div class="code-line">(config, args) => {</div>
                  <code-editor
                    v-model="formData.requestFunc"
                    mode="javascript"
                    height="150px"
                  ></code-editor>
                ...
                <el-collapse-item
                  :title="$t('fm.datasource.edit.responseSuccess')"
                  name="2"
                >
                  <div class="code-desc" v-if="$i18n.locale == 'zh-cn'">
                    // {{ $t('fm.datasource.edit.annotation') }}
                  </div>
                  <div class="code-line">(res, args) => {</div>

                  <code-editor
                    v-model="formData.responseFunc"
                    mode="javascript"
                    height="150px"
                  ></code-editor>
                  <div class="code-line">}</div>
                ...
                <el-collapse-item
                  :title="$t('fm.datasource.edit.requestError')"
                  name="3"
                >
                  <div class="code-line">(error) => {</div>

                  <code-editor
                    v-model="formData.errorFunc"
                    mode="javascript"
                    height="150px"
                  ></code-editor>
                  <div class="code-line">}</div>
...
export default {
        ...
        name: [
          { required: true, message: ' ' },
          {
            validator: (rule, value, callback) => {
              let currentItem = this.historyList.find(
  ...
  methods: {
    _getAttributeFunction(funcStrs) {
      let matchReg1 = /(?<=args.)\w*(?=[\s|,|;])/g // 匹配 args.a 形式的参数
      let matchReg2 = /(?<=args\[['"])\w*(?=['"]\])/g // 匹配 args[] 形式的参数
      let matchReg3 = /(?<=\{)[\s+\w*\s+,{0|1}]+(?=\}\s+=\s+args)/g // 匹配 {} = args 形式的参数
      ...
      funcStrs.forEach((funcStr) => {
        funcStr
          ?.match(matchReg1)
          ?.forEach((item) => item && args.add(item.trim()))
        funcStr
          ?.match(matchReg2)
          ?.forEach((item) => item && args.add(item.trim()))
        funcStr
          ?.match(matchReg3)
          ?.forEach((item) =>
            item.split(',').forEach((sitem) => sitem && args.add(sitem.trim()))
          )
      })
    ...
    requestTest(args = {}) {
      ...
      if (this.formData.requestFunc) {
        const requestDynamicFunc = new Function(
          'config',
          'args',
          this.formData.requestFunc
        )
      ...
      axios(options)
        .then((res) => {
          try {
            let data = new Function('res', 'args', this.formData.responseFunc)(
              res.data,
              args
            )
    ...
    handleSave() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          this.list[
            this.list.findIndex((item) => item.key === this.selectKey)
          ] = options
    ...
    loadForm() {
      ...
      this.formData = {
        ...
        params:
          (currentData.params &&
            Object.keys(currentData.params).map((item) => ({
              key: item,
              value: currentData.params[item],
            }))) ||
          [],
        headers:
          (currentData.headers &&
            Object.keys(currentData.headers).map((item) => ({
              key: item,
              value: currentData.headers[item],
            }))) ||
          [],
    ...
    handleRemove(index) {
      ...
      this.$confirm(
        `${this.$t('fm.datasource.message.confirmRemove')} [${
          currentData.name
        }] ?`,
        '',
        {
