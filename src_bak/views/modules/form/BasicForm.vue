        ...
        <el-row :gutter="15">
          ...
          <el-col :span="24">
            <el-form-item
              label="所属数据库"
              prop="dataSource.id"
              :rules="[{ required: true, message: '所属数据库不能为空', trigger: 'blur' }]"
            >
              <el-tree-select
                ref="dataSourceTree"
                value-key="id"
          ...
          <el-col :span="24">
            <el-form-item
              label="是否自动建表"
              prop="autoCreate"
              :rules="[{ required: true, message: '是否自动建表不能为空', trigger: 'blur' }]"
            >
              <el-radio-group
                :disabled="method !== 'add'"
          ...
          <el-col :span="24">
            ...
            >
              ...
              >
                <template #append>
                  <el-button :disabled="method !== 'add'" @click="createTableName"
                    >生成随机表名</el-button
                  >
                </template>
              </el-input>
...
export default {
  ...
  watch: {
    'inputForm.dataSource.id': {
      handler(val) {
        if (val) {
          this.inputForm.dataSource.enName = this.$refs.dataSourceTree?.getNode(val)?.data?.enName;
          this.inputForm.dataSource.type = this.$refs.dataSourceTree?.getNode(val)?.data?.dbType;
        }
        if (val === 'master') {
          this.inputForm.dataSource.enName = 'master';
        }
        makeFormService
          .getTableList({ 'dataSource.enName': this.inputForm.dataSource.enName })
          .then((data) => {
