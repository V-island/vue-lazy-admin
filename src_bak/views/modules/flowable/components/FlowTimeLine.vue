          ...
          <el-timeline  v-if="historicTaskMap.size">
              ...
              <el-timeline-item color="#3f9eff" :key="index" v-for="(arra, index) in historicTaskMap"  :timestamp="moment(arra[1][0].histIns.endTime).format('YYYY-MM-DD')" placement="top">
                ...
                <el-card  class="flow">
                  ...
                  <el-row :gutter="25">
                  <template :key="index"  v-for="(act, index) in arra[1]" >
                  ...
                  <div class="item">
                        ...
                        <span class="value">
                          <el-tag :type="act.comment.level"   effect="dark" >{{act.comment.status}} </el-tag>
                        </span>
                  </div>
                  <div class="item">
  ...
  computed: {
    historicTaskMap () {
      ...
      for(var index = list.length - 1; index >= 0; index--){
        let act = list[index]
        let key = act.histIns.activityId + act.histIns.startTime
        let val = map.get(key)
        if (val) {
          val.push(act)
        } else {
