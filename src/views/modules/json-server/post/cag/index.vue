<template>
    <CommonPage show-footer :title="pageInfo.title">
        ...
        <JsonTable ref="$table" row-key="title" v-model:query-items="queryItems" :extra-params="extraParams"
...
onMounted(() => {
    watchEffect(async () => {
        if (modalVisible.value == true) {
            const { data } = await pageInfo.api.list()
            pageInfo.cags = [{ title: '根目录', name: 'root' }, ...data];
        }
    })
})
