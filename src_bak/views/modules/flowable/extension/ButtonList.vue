<template>
  <div class="page">
      <el-form  :inline="true" class="query-form m-b-10" ref="searchForm" :model="searchForm" @keyup.enter="refreshList()" @submit.prevent>
            <!-- 搜索框-->
		     ...
		     <el-form-item prop="code">
                <el-input  v-model="searchForm.code" placeholder="编码" clearable></el-input>
		     </el-form-item>
          <el-form-item>
      ...
      <div class="jp-table">
        <vxe-toolbar ref="buttonToolbar" :refresh="{query: refreshList}" export print custom>
          <template #buttons>
            <el-button v-if="_hasPermission('extension:button:add')" type="primary"  icon="DocumentAdd" @click="add()">新建</el-button>
            <el-button v-if="_hasPermission('extension:button:edit')" type="warning"  icon="edit" @click="edit()"
            :disabled="$refs.buttonTable && $refs.buttonTable.getCheckboxRecords().length !== 1" >修改</el-button>
            <el-button v-if="_hasPermission('extension:button:del')" type="danger"  icon="delete" @click="del()"
            :disabled="$refs.buttonTable && $refs.buttonTable.getCheckboxRecords().length == 0" >删除</el-button>
          </template>
      </vxe-toolbar>
        <div class="jp-table-body">
          <vxe-table
            ...
            <vxe-column title="操作" width="200px" fixed="right" align="center">
              <template  #default="scope">
                <el-button v-if="_hasPermission('extension:button:view')"  type="primary" text  size="small" icon="view"  @click="view(scope.row.id)">查看</el-button>
                <el-button v-if="_hasPermission('extension:button:edit')"  type="primary" text  size="small" icon="edit"  @click="edit(scope.row.id)">修改</el-button>
              </template>
            </vxe-column>
          </vxe-table>
        </div>
