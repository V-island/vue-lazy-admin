<template>
	<el-dialog v-model="dialogVisible" title="人员选择" :width="type==1?680:460" destroy-on-close append-to-body @closed="$emit('closed')" draggable>
		...
		<template v-if="type==1">
			<div class="user-select">
											...
											<template #default="{node, data}">
												<span class="custom-tree-node el-tree-node__label">
													...
													<el-icon :size="16" class="m-r-5">
															<qiye v-if="data.type === '1'" :style="{color: $TOOL.data.get('IS_PRIMARY_TENANT') && data.tenantDTO && $utils.getTenantColorById(data.tenantDTO.id)}"></qiye> 
															<bumen v-else :style="{color: $TOOL.data.get('IS_PRIMARY_TENANT') && data.tenantDTO && $utils.getTenantColorById(data.tenantDTO.id)}"></bumen> 
													 </el-icon>
													{{ node.label}}
													</span>
												</span>
							...
							<div class="user-select__user__list">
								<el-scrollbar ref="userScrollbar">
									<el-tree class="menu"
											...
											:show-checkbox="limit > 1" check-on-click-node 
				...
				<div class="user-select__toicon"><el-icon><arrow-right /></el-icon></div>
					...
					<ul>
						<el-scrollbar>
							<li v-for="(item, index) in selected" :key="item.id">
								<span class="name">
									<el-avatar v-if="$TOOL.data.get('IS_PRIMARY_TENANT')" :size="30" :style="{background: $utils.getTenantColorById(item.tenantId) + '!important'}" >{{item.name.substring(0,1)}}</el-avatar>
									<el-avatar v-else>{{item.name.substring(0,1)}}</el-avatar>
									<label>{{item.name}}</label>
								</span>
								<span class="delete">
									<el-button text type="primary" icon="delete" circle size="small" @click="deleteSelected(index)"></el-button>
								</span>
							</li>
						</el-scrollbar>
					</ul>
...
<style scoped>
	...
	.user-select__selected header {height:43px;line-height: 43px;border-bottom: 1px solid var(--el-border-color-light);padding:0 15px;font-size: 16px;}
	...
	.user-select__selected li .name .el-avatar {margin-right: 10px;}
