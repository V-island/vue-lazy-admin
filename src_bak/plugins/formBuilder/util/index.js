...
export const updateStyleSheets = (sheets, head) => {
  ...
  while (stylesheets.cssRules.length > index) {
    if (
      stylesheets.cssRules[index].selectorText &&
      stylesheets.cssRules[index].selectorText.indexOf(head) === 0
    ) {
...
export const fixDraggbleList = (list, newIndex) => {
  // 处理拖拽过程中，列表值和返回的newIndex不一致问题
