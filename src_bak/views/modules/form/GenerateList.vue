  ...
  <div class="page">
    ...
    >
      ...
      <el-form-item
        ...
        :prop="option.model"
      >
        <user-select v-if="option.type === 'user'" v-model="params[`${option.model}`]">
        </user-select>
        <OfficeSelect v-else-if="option.type === 'office'" v-model="params[`${option.model}`]" />
        <AreaSelect v-else-if="option.type === 'area'" v-model="params[`${option.model}`]" />
        <el-select v-else-if="option.type === 'dict'" v-model="params[`${option.model}`]">
          <el-option
      ...
      >
        ...
        <el-input v-else :placeholder="option.name" v-model="params[`${option.model}`]"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-dialog title="导入Excel" v-model="isImportCollapse" draggable>
      <el-form :inline="true" class="query-form m-b-10" ref="importForm">
        <el-form-item>
          <el-button type="default" @click="downloadTpl()">下载模板</el-button>
        </el-form-item>
    ...
    <div class="jp-table" :style="showSearchBtn ? 'height:calc(100% - 50px)' : 'height:100%'">
      <vxe-toolbar ref="tableToolbar" :refresh="{ query: refreshList }" export print custom>
        <template #buttons>
          <el-button
            v-if="$route.query.previewMode || _hasPermission(`form:${tableName}:add`)"
            type="primary"
            icon="DocumentAdd"
            @click="add()"
            >新建</el-button
          >
          <el-button
            v-if="$route.query.previewMode || _hasPermission(`form:${tableName}:edit`)"
            type="warning"
            icon="edit"
            @click="edit()"
            :disabled="$refs.table && $refs.table.getCheckboxRecords().length !== 1"
            plain
            >修改</el-button
          >
          <el-button
            v-if="$route.query.previewMode || _hasPermission(`form:${tableName}:del`)"
            type="danger"
            icon="delete"
            @click="del()"
            :disabled="$refs.table && $refs.table.getCheckboxRecords().length === 0"
            plain
            >删除
          </el-button>
          <el-button
            ...
            v-for="(item, index) in options.config.actions.filter((action) => {
              return (
                ...
                ($route.query.previewMode || !action.auth || _hasPermission(action.auth))
              );
            })"
            :key="index"
            icon="link"
        ...
        >
            ...
            <template #default="scope">
              ...
              <div v-else-if="option.type === 'checkbox' && option.options.remote === 3">
                ...
                >
                  ...
                  <span
                    v-if="index + 1 !== JSON.parse(scope.row[`${option.model}`] || '[]').length"
                  >
                    |
                  </span>
                </span>
              ...
              <div v-else>
                <div v-if="index === 0">
                  ...
                  <el-link
                    type="primary"
                    :underline="false"
                    v-else-if="$route.query.previewMode || _hasPermission(`form:${tableName}:view`)"
          ...
          <vxe-column title="操作" width="200px" fixed="right" align="center">
            <template #default="scope">
              <el-button
                type="primary"
                v-if="$route.query.previewMode || _hasPermission(`form:${tableName}:view`)"
                text
                icon="view"
                size="small"
                @click="view(scope.row.id)"
  ...
  methods: {
    ...
    go(item, row) {
      ...
      if (isURL(item.link)) {
        this.$router.push({
          path: '/form/explorer',
          query: { title: item.name, iframeUrl: `${item.link}?${item.paramKey}=${keyValue}` },
        });
      } else {
        this.$router.push({
