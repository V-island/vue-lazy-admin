<template>
  <CommonPage show-footer :title="pageInfo.title">
    ...
    <JsonTable ref="$table" :row-key="pageInfo.rowKey" v-model:query-items="queryItems" :extra-params="extraParams"
      :scroll-x="1200" :columns="pageInfo.columns" :get-data="pageInfo.api.list" @on-checked="onChecked"
      @on-data-change="(data) => (tableData = data)" :remote="false">
      <template #queryBar>
          ...
          <div>
            ...
            <n-form-item label="分类" path="category" :rule="{
              ...
              <n-tree-select :options="pageInfo.cags" label-field="title" key-field="name"
                v-model:value="modalForm.category" filterable :default-value="pageInfo.cags[0].name || ''" />
            </n-form-item>
...
import { NForm, NFormItem, NButton, NSwitch, NSpace, NTooltip, NDropdown, NTreeSelect, NUpload, NUploadDragger, NEllipsis, NImage } from 'naive-ui'
import { formatDateTime, renderIcon, isNullOrUndef, lStorage, convertImgUrl } from '@/utils'
...
const pageInfo = reactive({
  ...
  columns: [
    ...
    },
    {
      ...
      render(row) {
        return pageInfo.cags.find(c => c.name == row.category)?.title || row.category
      }
    },
