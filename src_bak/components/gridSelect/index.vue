<template>
    ...
    <el-input :placeholder="placeholder" :disabled="disabled" :size="size" :readonly="true" style="width:100%; line-hight:40px" v-model="name" class="input-with-select" >
       <template #append>
        <el-button :disabled="disabled" @click="showSelectDialog"  icon="Search" />
      </template>
    </el-input>
    <v-dialog
    :title="title"
    ...
    <el-row :gutter="15">
    <el-col :span="19">
    <el-form  :inline="true" ref="searchForm" :model="searchForms" class="query-form"  @keyup.enter="refreshList()" @submit.prevent>
         <el-form-item v-for="(search,index) in searchs" :key="index" :prop="search.prop">
          <el-input  v-model="searchForms[index]" :placeholder="search.label" clearable></el-input>
         </el-form-item>
        <el-form-item>
        ...
        width="50">
          <template #default="scope">
           <el-radio-group v-model="radioSelected">
              <el-radio :label="scope.row.id" @change="getTemplateRow(scope.$index,scope.row)"><span></span></el-radio>
           </el-radio-group>   
          </template>
      </el-table-column>
      <el-table-column
...
<script>
  export default {
    ...
    computed: {
      radioSelected () {
       return  this.dataListAllSelections[0] && this.dataListAllSelections[0].id
      }
    },
    watch: {
      modelValue: {
