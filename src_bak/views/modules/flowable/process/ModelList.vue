  ...
  <div class="page">
    ...
    >
      <el-form-item label="模型名称" prop="filterText">
        <el-input v-model="searchForm.filterText" placeholder="请输入关键词"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <div class="jp-table">
      <vxe-toolbar ref="modelToolbar" :refresh="{ query: refreshList }" export print custom>
        <template #buttons>
          ...
          <el-button
            ...
            icon="delete"
            @click="del()"
            :disabled="$refs.modelTable && $refs.modelTable.getCheckboxRecords().length === 0"
            plain
            >删除</el-button
          >
          <!-- <el-button
            type="success"
            :disabled="$refs.modelTable && $refs.modelTable.getCheckboxRecords().length !== 1"
            @click="setCategory()"
            >设置分类</el-button
          > -->
        </template>
      ...
      <div class="jp-table-body">
        ...
        >
          ...
          <vxe-column title="更新时间" field="lastUpdated" sortable> </vxe-column>
          <vxe-column title="操作" width="220px" fixed="right" align="center">
            <template #default="scope">
              <el-button type="primary" text @click="handleExpandEvent(scope.row)"
                >节点拓展</el-button
              >
              <el-button type="primary" text @click="design(scope.row)">设计</el-button>
              <el-button type="primary" text @click="deploy(scope.row)">发布</el-button>
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <!-- <el-dropdown-item v-if="scope.row.procDef.suspended === true"
                      ><el-button type="primary" text @click="active(scope.row.procDef)"
                        >激活</el-button
                      ></el-dropdown-item
                    > -->
                    <!-- <el-dropdown-item v-if="scope.row.procDef.suspended === false"
                      ><el-button type="primary" text @click="suspend(scope.row.procDef)"
                        >挂起</el-button
                      ></el-dropdown-item
                    > -->
                    <!-- <el-dropdown-item
                      ><el-button type="primary" text @click="exportXML(scope.row)"
                        >导出</el-button
                      ></el-dropdown-item
                    > -->
                    <!-- <el-dropdown-item>
                      <el-button type="primary" text @click="copy(scope.row.id)">复制</el-button>
                    </el-dropdown-item> -->
                    <el-dropdown-item>
                      <el-button type="danger" text @click="del(scope.row.id)">删除</el-button>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
      ...
      <v-dialog title="查看进度" :close-on-click-modal="true" v-model="visible" height="600px">
        <iframe
          :src="processPhotoUrl"
          frameborder="0"
          scrolling="auto"
      ...
      <!-- 弹窗, 新增 / 修改 -->
      <ModelForm ref="modelForm" @create="create" />
      <!--模型编辑-->
      <design-form ref="designForm" @refreshList="refreshList" />
      <CategoryForm ref="categoryForm" @refreshList="refreshList" />
      <!-- 节点功能拓展 -->
      <ExpandDialog :config="expandConfig" />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    refreshList() {
      ...
      modelService
        ...
        .then((data) => {
          this.dataList = data.records.slice(
    ...
    exportXML(row) {
      window.open(
        `${process.env.VUE_APP_SERVER_URL}/rest/models/${row.id}/bpmn20?version=` +
          new Date().getTime(),
      );
    },
    // 部署
    ...
    // 节点功能拓展
    handleExpandEvent(row) {
      this.expandConfig.show = true;
      this.expandConfig.title = '节点功能拓展';
      this.expandConfig.params = {
        id: row.procDef.id,
      };
    },
  },
